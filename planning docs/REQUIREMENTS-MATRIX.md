...
| FR-24 | Conversational Agentforce agent (Veridact Auditor Copilot) over the four governed audit actions | Part 2 / Penasoto → Agentforce | agent `Veridact_Auditor_Copilot_v4` **Active** on `mortagate-de`; topic "Summarize Rule Violations" carries the 4 governed `GenAiFunction` Apex actions (Diagnose Loan Audit, Summarize Case, List Missing Evidence, Draft Finding Text) — each bundle now ships `input/schema.json` + `output/schema.json` deriving `auditCaseId` (`lightning__recordIdType`, planner-extracted via `einstein:description`); ungoverned standard actions removed from both topics (ADR-16) | ✅ **live-verified**: planner extracts `auditCaseId` from utterance, launches `List_Missing_Evidence` + `Summarize_Case`, fresh `Agent_Action_Log__c` `Initiated` rows written (log-before-execute) | ✅ |
...
| FR-32 | Automated "second pass behind the human auditor" — a scheduled batch replays every eligible (open + never-replayed OR stale) loan through the existing deterministic kernel (no LLM), surfacing exceptions in the queue (escalate-only risk tier) | product pitch, ADR-21, ADR-27 | `SecondPassSweepBatch` (`Database.Batchable`+`Schedulable`, chunk size 1; reuses `ReplayService`; staleness from `LoanDiagnosisService`; escalate-only via `deriveRiskTier`/`rankOf`); on-demand `scripts/run-second-pass.apex` | `SecondPassSweepBatchTest` (8) — violation→High+in-queue, clean→recorded-not-demoted, current-skipped, stale→re-run, escalate-only rank, bulk-200 select+per-chunk budget — 8/8, 98% cov on `mortagate-de`. Nightly schedule LIVE on `mortagate-de` via `SecondPassSweepBatch.schedule('Veridact Second Pass - Nightly','0 0 2 * * ?')` — CronTrigger `08egL00000bzH8HQAU`, State WAITING, fires 02:00 org-local (America/Los_Angeles) nightly | ✅ |
| FR-33 | Copilot action **Summarize_Case** — read-only plain-English case briefing (loan snapshot: amount/property value/DTI·LTV·FICO as whole-number percents; replay Pass/Fail/Unverifiable counts + named broken rules; evidence linked/missing incl. required trailing docs; status/risk/SLA), guardrailed (never "denied", defers to auditor, staleness notice), log-before-execute dual-write | product "second pass" sidebar, ADR-16/21/26 | `CaseSummaryService` (`@InvocableMethod` + `@AuraEnabled` fallback; hoisted reads; reuses `AuditSoql.latestChecksFor`, `RuleNarration`) + `GenAiFunction:Summarize_Case` | `CaseSummaryServiceTest` (4: hero DTI-fail + 2 required-missing, no-replay Partial, fail-closed-no-orphan, bulk-200 ≤5 SOQL/3 DML + dual-write) — 4/4 on `mortagate-de` | ✅ |
| FR-34 | Copilot action **List_Missing_Evidence** — read-only stip-clearance list: every non-Linked `Evidence_Item__c` ranked required-missing first, then optional-missing, then Unverifiable replay checks; guardrailed; log-before-execute dual-write | product "second pass" sidebar, ADR-16/21/26 | `MissingEvidenceService` (`@InvocableMethod` + `@AuraEnabled` fallback; hoisted reads; reuses `AuditSoql.latestChecksFor`, `RuleNarration`) + `GenAiFunction:List_Missing_Evidence` | `MissingEvidenceServiceTest` (4: hero ranking + unverifiable, clean-nothing-outstanding, fail-closed-no-orphan, bulk-200 ≤4 SOQL/3 DML + dual-write) — 4/4 on `mortagate-de` | ✅ |
| FR-35 | Copilot action **Draft_Finding_Text** — read-only DRAFT finding narrative from a failing `Replay_Check__c` (by id or rule code → latest batch): plain rule name+code, expected-vs-actual, rationale, evidence linkage; suitable for `Finding__c.Description__c`. **Drafts text only — never inserts/updates a Finding** (human accepts via `findingDetail`, FR-27); guardrailed; log-before-execute dual-write | product "second pass" sidebar, FR-27, ADR-16/21/26 | `FindingDraftService` (`@InvocableMethod` + two `@AuraEnabled` fallbacks; hoisted reads; reuses `AuditSoql`, `RuleNarration`) + `GenAiFunction:Draft_Finding_Text` | `FindingDraftServiceTest` (5: draft-by-id, draft-by-rule-code, evidence-line, no-check Partial, **0 Finding inserted**, bulk-200 ≤3 SOQL/3 DML + dual-write) — 6/6 on `mortagate-de` | ✅ |
| FR-36 | Draft evidence-request message for missing documents — text only, never sends; conversational surface rides List_Missing_Evidence + LLM composition (ADR-28); bespoke service is the UI/programmatic path | pitch (stip-clearance), ADR-16/21/26/28 | `EvidenceRequestService` (`@InvocableMethod` + `@AuraEnabled`, ADR-26 dual-write, fail-closed) + schema-complete `genAiFunctions/Draft_Evidence_Request` bundle (deployed, unbound from topics by ADR-28) | `EvidenceRequestServiceTest` (PNB + dual-write + bulk-200 + guardrails) 8/8 on `mortagate-de`; conversational draft verified live via List_Missing_Evidence grounding | ✅ |
