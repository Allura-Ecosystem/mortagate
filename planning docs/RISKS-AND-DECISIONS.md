...
**Also quarantined via `.forceignore` (per ADR-23 / R-10):** `EmployeeCopilotPlanner*` (stock SF Employee copilot, Active in org — deploy cannot overwrite an active agent) and `Veridact_Auditor_Copilot_v4_RuntimeSelection*` (AiEvaluationDefinition missing its required Version sibling; re-enable once authored). All Veridact bots, planner bundles, GenAiFunctions and GenAiPlugins deploy fine — ADR-23's "not org-portable" applies to *active/stock* agents, not Agentforce metadata wholesale.
