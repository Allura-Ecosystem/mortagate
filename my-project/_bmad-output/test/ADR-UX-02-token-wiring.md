...
> **UPDATE 2026-07-02 (Woz / G1 stage-5) — STAGE 5 COMPLETE: tokens wired document-wide, ALL component hex removed, guard GREEN, deployed.**
> Mechanism chosen: `loadStyle(this, veridactTokens)` in `connectedCallback` of the **7 exposed root components** (auditAdmin, auditAnalytics, auditQueue, caseReview, findingDetail, policyVersions, signoffReceipt). `loadStyle` injects the static resource at the document level, so its `:root` block sets the custom properties globally; they then **inherit** down the composed tree into every child (custom-property inheritance is the one thing that crosses shadow-DOM boundaries, so nested base components — e.g. `c-risk-badge` inside `lightning-datatable` native shadow — receive them too). This is why the 4 non-exposed leaves (auditMetricCards, auditQueueFilters, glossaryTerm, riskBadge) did **not** need their own `loadStyle` — minimal surface vs. the "all 12" note below, justified by the inheritance model. `veridactTokens.css` selector changed `:root` → `:root, :host` so the same sheet also resolves if ever injected into a shadow root (native mode). **All hex fallbacks dropped** (`var(--veridact-ink, #1F1E1C)` → `var(--veridact-ink)`) AND all remaining **raw** hexes tokenized; 6 new tokens added to the static resource (`--veridact-on-accent`, `--veridact-accent-strong-hover`, `--veridact-border-subtle`, three `*-border` tint hairlines). Guard `node scripts/check-brand-tokens.mjs` → **exit 0** (11 stylesheets clean); `sfdx-lwc-jest` → **53/53 green** (a11y matchers intact). Deployed piecewise (`LightningComponentBundle` + `StaticResource`) to `mortagate-de` → **exit 0, all Changed** (veridactTokens `081gL000002BSW5QAO`, lastModified 2026-07-02). Guard message refined (Fowler finding) to distinguish tokenized-fallback vs raw hex. **Failure mode:** if `loadStyle` ever fails (resource missing/blocked), `--veridact-*` do not resolve and, with fallbacks now gone, components lose brand color — but they degrade *legibly*: unset `color` inherits the UA dark default (no white-on-white), unset `background` falls to transparent/card. No invisible text. **G3 (browser visual re-check) is now WARRANTED** since runtime paint switched from fallback-driven to token-driven.
