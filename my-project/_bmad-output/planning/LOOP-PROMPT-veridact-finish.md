...
3. **Dispatch the right specialist** (Task tool): Woz builds Apex/LWC, Knuth owns schema/rule data, Hightower owns deploy/CI, Pike reviews UI, Scout recons unknowns. Inject relevant Brain traces into every dispatch prompt. Specialists work ONLY in the main checkout — never `.claude/worktrees/*`.
...
8. **Stop check** (deterministic — evaluate in order):
   ...
   - Otherwise → schedule the next iteration with this same prompt.
